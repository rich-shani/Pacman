...
function beginstep2() {
	if dead = 0 and global.start = 0 and chomp = 0{
	...
	////eating a dot
	if dead = 0 and global.start = 0 and Pac.prohibit < 2{
	...
	if place_meeting((16*(round(x2/16))),(16*(round(y2/16))),Power){
	    ...
	    if global.players = 2{if Pac.prohibit = 2{Pac.prohibit = 0;}else{Pac.prohibit = 1;}} fright = 1; Pac.alarm[0] = Pac.frighttime; Pac.chompcount = 0;
	    }
	    Blinky.visible = true; Pinky.visible = true; Inky.visible = true; Clyde.visible = true; Pac.pluseat  = 0;
	    Pac.Pac.plus = -1; Pac.plus2 = 0;
	    Pac.timer = Pac.timerstart + 1; if global.plus = 1{Pac.Pac.plus = irandom(7); if Pac.frighttime > 0 and (global.game < 2 and global.lvl > 2) or (global.game = 2 and global.lvl > 1){Pac.plus2 = irandom(1);}}
	    with Blinky{aboutface = 1; if state<2 and Pac.frighttime > 0{if Pac.Pac.plus = 0{if state=1{state=0}}else{state = 1}}}
	    with Pinky{aboutface = 1; if state<2 and Pac.frighttime > 0{if Pac.Pac.plus = 1{if state=1{state=0}}else{state = 1}}}
	    with Inky{aboutface = 1; if state<2 and Pac.frighttime > 0{if Pac.Pac.plus = 2{if state=1{state=0}}else{state = 1}}}
	    with Clyde{aboutface = 1; if state<2 and Pac.frighttime > 0{if Pac.Pac.plus = 3{if state=1{state=0}}else{state = 1}}}
	    Pac.Pac.plus = -1;
	    bonked2 = 0;
	if global.lvl > 0{
	...
	////Pac.finish
	if global.lvl > 0{if global.start = 0{if dead = 0{
	if (Pac.dotcount = global.dottotal or Pac.dotcount > global.dottotal) and Pac.finish = 0{
	Pac.finish = 1; alarm[11] = 60; alarm[1] = -1; Blinky.aboutface = 0; Pinky.aboutface = 0; Inky.aboutface = 0; Clyde.aboutface = 0;
	Blinky.speed = 0; Pinky.speed = 0; Inky.speed = 0; Clyde.speed = 0; Pac.speed = 0;
	}}}}
	////eating a fruit
	if dead = 0 and Pac.prohibit < 2{
	...
	if global.plus = true{
	    ...
	    fright = 1; Pac.alarm[0] = bonustime; Pac.chompcount = 0; Pac.plus2 = 0; Pac.pluseat  = 1;
	    ...
	    with Blinky{aboutface = 1; if state<2 and bonustime > 0{state = 1}}
	    with Pinky{aboutface = 1; if state<2 and bonustime > 0{state = 1}}
	    with Inky{aboutface = 1; if state<2 and bonustime > 0{state = 1}}
	    with Clyde{aboutface = 1; if state<2 and bonustime > 0{state = 1}}
	} 
	    exit}}
	}}
	}
	...
	if global.start = true or Pac.finish > 0{hspeed2 = 0; vspeed2 = 0;
	}
